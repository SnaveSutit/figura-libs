function update(hip, knee, foot, target)
	...
	local len_leg =
		math.clamp((target - hip):length(), safty, len_femur + len_tibia - safty)

	local theta = math.atan(target.z / target.y)

	local knee_angle =
		(len_femur * len_femur) + (len_tibia * len_tibia) - (len_leg * len_leg)
	knee_angle = knee_angle / (2 * len_femur * len_tibia)
	knee_angle = math.acos(knee_angle)

	local alpha =
		(len_femur * len_femur) + (len_leg * len_leg) - (len_tibia * len_tibia)
